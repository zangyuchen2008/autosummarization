...
def get_extractin(doc,embedding_size,model, word_sifs, stop_words,word_idf,threshhold,\
    ...
    doc_embedding , _ = sentence_to_vec([doc],100,model,word_sifs,stop_words,)

    # find key word in doc
    tfidfs = get_tfidfs(doc,word_idf,stop_words,5)
    sens_keyword_num = []
    for sen in spl_sens_cleared:
        words = list(jieba.cut(sen))
        keyword_num = sum([ 1 for w in words if w in dict(tfidfs)])
        sens_keyword_num.append(keyword_num)
    sens_embedding_ziped = list(zip(spl_sens_cleared,sens_embedding1,sens_keyword_num,range(len(spl_sens_cleared))))
    
    sorted_sens=[]
    for sen, vec, keyword_num ,original_pos in sens_embedding_ziped:
        if title:
            # calculate title vec
            title_embedding , _ = sentence_to_vec([title],100,model,word_sifs,stop_words,)
...
但是，澳大利亚政府多次为其离岸移民拘留政策辩护，称这是事关国家安全的问题。澳政府表示，拒绝难民在澳大利亚定居的机会降低了人们前往澳大利亚的动机，防止了海上死亡，并使澳大利亚的边境更加安全。'''
    ...
    print('get_extractin1:',''.join([a[0] for a in get_extractin(doc,100,model,\
        word_sifs,stop_words,word_idf,4,0.1,0.4,'曾在中国学医的阿富汗青年 因受虐死于澳洲难民营',True)]))